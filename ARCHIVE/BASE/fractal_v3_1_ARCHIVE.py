...
class SystemicAnalyzer:
    ...
    def bateson_feedback(self):
        """
        Compute the Bateson Feedback metric for the 'close' price series.

        Bateson feedback captures the net directional bias in daily returns, defined as
        the difference between the average proportion of positive returns and the average
        proportion of negative returns over a fixed rolling window of 5 periods. Values
        lie in [ -1, 1 ], where values near +1 denote predominantly upward momentum, and
        values near -1 denote downward reinforcement.

        Returns
        -------
        float
            Net feedback measure between –1 and +1.
        """
    ...
    def lyapunov_proxy(self, lag=1):
        """
        Compute a proxy for the maximal Lyapunov exponent of the 'close' price series.

        This proxy estimates the average exponential rate of divergence between nearby
        trajectories in phase space. Log‑returns are differenced at the specified lag,
        and the mean of log1p(abs(differences)) is returned as an approximation of λ.

        Parameters
        ----------
        lag : int, optional
            Time lag over which to compute differences, default is 1.

        Returns
        -------
        float
            Approximate Lyapunov exponent proxy λ.
            • λ > 0 indicates chaotic dynamics with sensitive dependence on initial conditions.
            • λ = 0 suggests neutral stability.
            • λ < 0 implies convergent, stable dynamics.
        """
