'''
──────────────────────────────────────────────────────────────────────────────────────────────────────────────────
                                   ADAPTIVE FRACTAL ANALYSIS PIPELINE v2
──────────────────────────────────────────────────────────────────────────────────────────────────────────────────
            ...
            ▼                                                           ▼
 ┌──────────────────────────┐                                  ┌──────────────────────────┐
 │ get_daily_data()         │                                  │ plot_with_quantification │
 │  • download EOD prices   │                                  │  • fit trend + ±σ bands  │
 │  • resample & clean      │                                  │  • highlight deviations  │
 └──────────┬───────────────┘                                  └──────────────────────────┘
            │
            ▼
 ┌────────────────────────────┐
 ...
 │  • detect latent regimes   │
 └──────────┬─────────────────┘
            │
            ▼
 ┌────────────────────────────┐
 │  TailReversionAnalyzer     │
 │  .analyze()                │
 │  • tail energy gradients   │
 │  • mimetic tension         │
 │  • quantum collapse        │
 │  • fractal tail memory     │
 │  • adaptive tail probs     │
 ...
 ┌────────────────────────────┐
 │ Logger + log_method        │
 │  • log JSON to file/console│
 │  • track per-method time   │
 │  • log total execution     │
 └────────────────────────────┘
...
class SystemicAnalyzer:
    ...
    def ashby_homeostasis(self, window=10):
        """
        Measures the stability of the system using the coefficient of variation.

        Parameters:
        - window (int): The rolling window size for mean and standard deviation (default is 10).

        Returns:
        - float: Average coefficient of variation over time.
                 Lower values indicate more stability and homeostasis.
                 Values are non-negative, typically ranging from 0 to above 1 depending on volatility.
        """
